...
def prism_beast_event(gamestate, position, direction, multiplier, whiff, include_padding_index=True):
    ...
    event = {
        "index": len(gamestate.book.events),
        "type": EventConstants.PRISM_BEAST.value,
        "position": {"reel": int(position["reel"]), "row": int(position["row"]) + off},
        "direction": direction,
...
def prism_path_event(gamestate, source, direction, cells, include_padding_index=True):
    ...
    for c in cells:
        out_cells.append(
            {
                "position": {"reel": int(c["position"]["reel"]), "row": int(c["position"]["row"]) + off},
                "multiplier": int(c["multiplier"]),
            }
        )
    event = {
        "index": len(gamestate.book.events),
        "type": EventConstants.PRISM_PATH.value,
